...
class ITunesDBPlaylistItem
{
	...
	void setName( String name )
	{
		if( this.contentCount == 0 ) // TODO what happens if playlist contains already some items ( and a name )
		{
			...
			ITunesDBContentItem content = new ITunesDBContentItem( ITunesDBContentItem.TITEL, 1, 
																															ITunesDBParser.stringToUTF16LittleEndian(name) );
	...
	public String toString()
	{
		return new StringBuffer("[tagSize] ").append(tagSize)
								...
								.append('\t').append("[songCount] ").append(songCount)
								.append('\t').append("[listType] ").append(listType)
								.toString();
	}
