...
public class ITunesDB
{
	 ...
	 *
	 * @param fileIdc vector containing file indices to summarize size
	...
	public long getFilesize(long fileIndex)
	{
		...
		if(!found) { logger.warning("found no filesize for file index: " + fileIndex ); }
	...
	private boolean tagSizesValid( boolean correct )
	{
		...
		{
			valid = false;
			logger.warning( "db record size invalid (record size is: " + this.recordSize
											+ ", but was calculated as:" + size + ")" );
			
			if(correct) {	this.recordSize = size; }
		}
		...
		{
			valid = false;
			logger.warning( "songlist holder size invalid ( record size is: " + this.songlistHolder.getRecordSize()
											+ ", but was calculated as:" + size + ")"	);
			
			if(correct) { this.songlistHolder.setRecordSize(size); }
		}
		...
		for( int i = 0; i < songlistHeader.getSongCount(); i++ )
		{
			...
			{
				valid = false;
				logger.warning( "song item invalid ( record size is: " + songItem.getRecordSize()
													+ ", but was calculated as:" + size + ")" );

				if(correct) { songItem.setRecordSize(size); }
			}
		} // for all songs
		...
		{
			valid = false;
			logger.warning( "playlist holder size invalid ( record size is:" + this.playlistHolder.getRecordSize() 
											+ ", but was calculated as:" + size + ")" );
			
			if(correct) { this.playlistHolder.setRecordSize(size); }
		}
		...
		for( int i = 0; i < this.playlistHeader.getPlaylistCount(); i++ )
		{
			...
			{
				valid = false;
				logger.warning( "playlistItem.recordSize invalid ( record size is:" + playlistItem.getRecordSize()
												+ ", but was calculated as:" + size + ")" );

				if(correct) { playlistItem.setRecordSize(size); }
			}			
		} // for all playlists
	 ...
	 * @param fileIdc Vector of fileindices to include in the playlist
	 */
	...
	ITunesDBPlaylistItem getPlaylist( String name )
	{
		...
		if(!found) {	logger.info("searching for playlist: " + name + " failed."); }
	...
	public String toString()
	{
		return new StringBuffer("[tagSize] ").append(tagSize)
						.append('\t').append("[recordSize] ").append(recordSize)
						.append('\t').append("[unknown3] ").append(unknown3)
						.append('\t').append("[unknown4] ").append(unknown4)
						.append('\t').append("[unknown5] ").append(unknown5)
						.toString();
	}
	...
	private boolean countsValid(boolean correct)
	{
		...
		for( int i = 0; i < this.playlistHeader.getPlaylistCount(); i++ )
		{
			...
			{
				valid = false;
				logger.warning( "playlist " + playlistItem.getName() + " song count invalid ( playlist song count is : " 
													+ playlistItem.getSongCount() + ", but playlist has :" 
													+ (playlistItem.getSongItems().size()-2) + " song items attached)" );

				if(correct) { playlistItem.setSongCount(playlistItem.getSongItems().size()-2); }			
			}
		} // for all playlists
			...
			for( Iterator indexIter = playlistItem.getSongItems().iterator(); indexIter.hasNext(); )
			{
					...
					)
				{
					...
					logger.warning( "playlist " + playlistItem.getName() + " contains a clip [index]: "
														+ ((ITunesDBPlaylistIndexItem)indexItem).getSongIndex()
