...
public class Song : ISearchable
{
	...
	public Gdk.Pixbuf CoverImage {
		set {
			...
			if (cover_image != null && cover_image != AppContext.CoverDB.DownloadingPixbuf)
	...
	*/
	...
	private bool ProcessDownloadedAlbumCover ()
	{
		...
		AppContext.CoverDB.ReplaceCover (AlbumKey, cover_image);

		AppContext.DB.UpdateSong (this);
		
		AppContext.DB.SyncAlbumCoverImageWithSong (this);
	...
	{
		try {
			tmp_cover_image = AppContext.CoverDB.GetAlbumCoverFromAmazon (this);
		} catch (WebException e) {
			/* Temporary web problem (Timeout etc.) - re-queue */
			Thread.Sleep (60000); /* wait for a minute first */
			AppContext.ActionThread.QueueAction (action);
	...
	{
		try {
			tmp_cover_image = AppContext.CoverDB.DownloadCoverPixbuf (new_cover_url);
		} catch {
			tmp_cover_image = null;
		}
	...
	private void GetCoverImage (Metadata metadata)
	{
		...
		if (AppContext.CoverDB.Covers.ContainsKey (key)) {
			cover_image = (Gdk.Pixbuf) AppContext.CoverDB.Covers [key];
			return;
		}
		...
		foreach (string fn in cover_filenames) {
			...
			if (cover.Exists) {
				cover_image = AppContext.CoverDB.AddCoverLocal (key, cover.ToString ());
		...
		if (metadata != null && metadata.AlbumArt != null) {
			cover_image = AppContext.CoverDB.AddCoverEmbedded (key, metadata.AlbumArt);
		...
		cover_image = AppContext.CoverDB.AddCoverDownloading (key);
