...
public class PlaylistWindow : Window
{
	...
	public PlaylistWindow () : base (WindowType.Toplevel)
	{
		...
		AppContext.DB.SongChanged += new SongDatabase.SongChangedHandler (HandleSongChanged);
		AppContext.DB.SongRemoved += new SongDatabase.SongRemovedHandler (HandleSongRemoved);
	...
	public void UpdateWindowVisibilityUI ()
	{
		if (WindowVisible) {
			...
			((Label) icon.show_window_menu_item.Child).LabelProp = AppContext.Catalog.GetString ("Hide _Window");
		} else {
			((Label) icon.show_window_menu_item.Child).LabelProp = AppContext.Catalog.GetString ("Show _Window");
		}
	}
	...
	private void SetupButtonsAndMenuItems (Glade.XML glade_xml)
	{
		...
		tooltips.SetTip (previous_button,
		                 AppContext.Catalog.GetString ("Play the previous song"), null);
		tooltips.SetTip (next_button,
		                 AppContext.Catalog.GetString ("Play the next song"), null);
		tooltips.SetTip (glade_xml ["add_album_button"],
		                 AppContext.Catalog.GetString ("Add an album to the playlist"), null);
		tooltips.SetTip (glade_xml ["add_song_button"],
			         AppContext.Catalog.GetString ("Add a song to the playlist"), null);
		...
		tooltips.SetTip (volume_button,
				 AppContext.Catalog.GetString ("Change the volume level"), null);

		int vol;
		try {
			vol = (int) AppContext.GConfClient.Get ("/apps/muine/volume");
		} catch {
			vol = 50;
		}
	...
	private void SetupPlayer (Glade.XML glade_xml)
	{
		try {
			...
			new ErrorDialog (String.Format (AppContext.Catalog.GetString ("Failed to initialize the audio backend:\n{0}\n\nExiting..."), e.Message));
	...
	private void SongChanged (bool restart)
	{
		if (playlist.Playing != IntPtr.Zero) {
			...
			if (song.Album.Length > 0)
				tip = String.Format (AppContext.Catalog.GetString ("From \"{0}\""), song.Album);
			else
				tip = AppContext.Catalog.GetString ("Album unknown");
			if (song.Performers.Length > 0)
				tip += "\n\n" + String.Format (AppContext.Catalog.GetString ("Performed by {0}"), StringUtils.JoinHumanReadable (song.Performers));
				
			if (song.CoverImage == null && !AppContext.CoverDB.Loading)
				tip += "\n\n" + AppContext.Catalog.GetString ("Drop an image here to use it as album cover");
			...
			Title = String.Format (AppContext.Catalog.GetString ("{0} - Muine Music Player"), song.Title);

			if (player.Playing)
				icon.Tooltip = artist_label.Text + " - " + title_label.Text;

			if (restart) {
				if (PlayerChangedSong != null)
			...
			Title = AppContext.Catalog.GetString ("Muine Music Player");
	...
	{
		if (playing) {
			tooltips.SetTip (play_pause_button, AppContext.Catalog.GetString ("Pause music playback"), null);
			...
			((Label) play_pause_menu_item.Child).LabelProp = AppContext.Catalog.GetString ("P_ause");

			icon.play_pause_menu_item_image.SetFromStock ("muine-pause", IconSize.Menu);
			((Label) icon.play_pause_menu_item.Child).LabelProp = AppContext.Catalog.GetString ("P_ause");

			icon.Tooltip = artist_label.Text + " - " + title_label.Text;
		} else if (playlist.Playing != IntPtr.Zero &&
		           player.Position > 0 &&
			   !had_last_eos) {
			tooltips.SetTip (play_pause_button, AppContext.Catalog.GetString ("Resume music playback"), null);
			...
			((Label) icon.play_pause_menu_item.Child).LabelProp = AppContext.Catalog.GetString ("Pl_ay");
			
			icon.Tooltip = null;
		} else {
			tooltips.SetTip (play_pause_button, AppContext.Catalog.GetString ("Start music playback"), null);
			play_pause_image.SetFromStock ("muine-play", IconSize.LargeToolbar);
	...
	{
		Song song = (Song) AppContext.DB.Songs [file];
	...
	private void HandleInformationCommand (object o, EventArgs args)
	{
		...
		if (song.Album.Length == 0)
			return;
		Album album = (Album) AppContext.DB.Albums [song.AlbumKey];
