...
public class InfoWindow : Window
{
	...
	public InfoWindow (string title) : base (IntPtr.Zero)
	{
		...
		int width;
		try {
			width = (int) AppContext.GConfClient.Get ("/apps/muine/information_window/width");
		} catch {
			width = 350;
		}
		...
		int height;
		try {
			height = (int) AppContext.GConfClient.Get ("/apps/muine/information_window/height");
		} catch {
			height = 300;
		}
...
//		viewport.ModifyBg (StateType.Normal, viewport.Style.Base (StateType.Normal));
	...
	private void HandleSizeAllocated (object o, SizeAllocatedArgs args)
	{
		...
		AppContext.GConfClient.Set ("/apps/muine/information_window/width", width);
		AppContext.GConfClient.Set ("/apps/muine/information_window/height", height);
	}
