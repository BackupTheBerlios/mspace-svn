...
public class AddSongWindow : Window
{
	...
	public AddSongWindow () : base (IntPtr.Zero)
	{
		...
		window.Title = AppContext.Catalog.GetString ("Play Song");

		int width;
		try {
			width = (int) AppContext.GConfClient.Get ("/apps/muine/add_song_window/width");
		} catch {
			width = 350;
		}
		...
		int height;
		try {
			height = (int) AppContext.GConfClient.Get ("/apps/muine/add_song_window/height");
		} catch {
			height = 300;
		}
		...
		AppContext.DB.SongAdded += new SongDatabase.SongAddedHandler (HandleSongAdded);
		...
		int i = 0;
		foreach (Song s in AppContext.DB.Songs.Values) {
	...
	private void HandleSizeAllocated (object o, SizeAllocatedArgs args)
	{
		...
		AppContext.GConfClient.Set ("/apps/muine/add_song_window/width", width);
		AppContext.GConfClient.Set ("/apps/muine/add_song_window/height", height);
	}
