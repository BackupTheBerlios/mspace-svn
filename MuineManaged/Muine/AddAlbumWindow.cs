...
public class AddAlbumWindow : Window
{
	...
	public AddAlbumWindow () : base (IntPtr.Zero)
	{
		...
		window.Title = AppContext.Catalog.GetString ("Play Album");

		int width;
		try {
			width = (int) AppContext.GConfClient.Get ("/apps/muine/add_album_window/width");
		} catch {
			width = 350;
		}
		...
		int height;
		try {
			height = (int) AppContext.GConfClient.Get ("/apps/muine/add_album_window/height");
		} catch {
			height = 300;
		}
		...
		AppContext.DB.AlbumAdded += new SongDatabase.AlbumAddedHandler (HandleAlbumAdded);
		AppContext.DB.AlbumChanged += new SongDatabase.AlbumChangedHandler (HandleAlbumChanged);
		AppContext.DB.AlbumRemoved += new SongDatabase.AlbumRemovedHandler (HandleAlbumRemoved);

		AppContext.CoverDB.DoneLoading += new CoverDatabase.DoneLoadingHandler (HandleDoneLoading);

		foreach (Album a in AppContext.DB.Albums.Values) 
					 ...
					 IntPtr handle)
	{
		...
		if (album.CoverImage != null)
			...
			r.Pixbuf = AppContext.CoverDB.DownloadingPixbuf;
		else
				       ...
				       IntPtr handle)
	{
		...
		if (album.Performers.Length > 0)
			performers = String.Format (AppContext.Catalog.GetString ("Performed by {0}"), StringUtils.JoinHumanReadable (album.Performers, 2));
	...
	private void HandleSizeAllocated (object o, SizeAllocatedArgs args)
	{
		...
		AppContext.GConfClient.Set ("/apps/muine/add_album_window/width", width);
		AppContext.GConfClient.Set ("/apps/muine/add_album_window/height", height);
	}
